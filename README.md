...
또한 포인트는 충전과 차감이 동시에 발생할 수 있는 상태 데이터이기 때문에,
초기 구현 이후 **동시성 제어 보완**을 진행했다.

특히 주문 생성과 포인트 충전 시 같은 사용자 포인트에 대해 동시에 접근할 수 있으므로,
현재는 **JPA 비관적 락(PESSIMISTIC_WRITE)** 을 적용해
잔액 정합성이 깨지지 않도록 보완했다.
...
- 동시에 2개의 주문 요청이 들어와 둘 다 잔액 검증을 통과하는 경우
- 동시에 충전 요청이 들어와 최종 잔액이 예상과 다르게 반영되는 경우
- 주문 생성과 포인트 차감이 분리되어 처리될 경우 데이터 정합성이 깨지는 경우

이 프로젝트에서는 아래 흐름을 하나의 논리 단위로 묶었다.
...
동시성 제어를 위해 **포인트 충전 / 주문 생성 시 Point 조회에 비관적 락(PESSIMISTIC_WRITE)** 을 적용했다.

이를 통해 같은 사용자의 포인트에 대해 동시에 여러 트랜잭션이 접근하더라도,
먼저 락을 획득한 트랜잭션이 끝날 때까지 다른 요청이 대기하게 하여
잔액 정합성을 보호하도록 구성했다.

### 확장 방향
- **낙관적 락(@Version)** 기반 충돌 제어
- 다중 인스턴스 환경에서는 **Redis 분산 락** 고려
- 외부 전송은 **Outbox Pattern + Kafka** 구조로 개선
- 실패 재처리 / 중복 방지는 **이벤트 기반 구조**에서 보완
...
- 동시성 테스트에서는 여러 스레드가 동시에 요청하는 상황을 만들어
  **포인트 잔액이 의도한 값으로 유지되는지**를 검증했다.
...
### 1) 동시성 제어 고도화
- 현재는 **비관적 락 기반으로 포인트 정합성**을 보장하고 있다.
- 다만 트래픽이 더 커지는 환경에서는 **낙관적 락** 또는 **Redis 분산 락** 등
  상황에 맞는 전략으로 확장할 수 있다.
